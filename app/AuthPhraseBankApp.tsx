...
type ApplicationProps = { repository: CloudPhraseRepository; contentInstaller: typeof installBundledSystemContent };
export function AuthPhraseBankApp({ fetcher = fetch, renderApp, createRepository = (request) => new CloudPhraseRepository(request), renderApplication }: { fetcher?: typeof fetch; renderApp?: (user: { username: string }) => ReactNode; createRepository?: (request: typeof fetch) => CloudPhraseRepository; renderApplication?: (props: ApplicationProps) => ReactNode }) {
  ...
  return <><div className="account-bar"><span>{state.user.username}</span><button type="button" onClick={() => { void logout(); }}>退出登录</button></div>
    {renderApp ? renderApp(state.user) : renderApplication ? renderApplication({ repository, contentInstaller: installBundledSystemContent }) : <PhraseBankApp repository={repository} contentInstaller={installBundledSystemContent} />}</>;
}
