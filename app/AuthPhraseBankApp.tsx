...
export function AuthPhraseBankApp({ fetcher = fetch, renderApp, createRepository = (request) => new CloudPhraseRepository(request) }: { fetcher?: typeof fetch; renderApp?: (user: { username: string }) => ReactNode; createRepository?: (request: typeof fetch) => CloudPhraseRepository }) {
  ...
  return <><div className="account-bar"><span>{state.user.username}</span><button type="button" onClick={() => { void logout(); }}>退出登录</button></div>
    {renderApp ? renderApp(state.user) : <PhraseBankApp repository={repository} />}</>;
}
