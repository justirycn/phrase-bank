...
export function PhraseBankApp({ repository, contentInstaller, initialScreen = "home", username, onLogout }: { repository?: Repository; contentInstaller?: (repository: Repository) => Promise<unknown>; initialScreen?: Screen; username?: string; onLogout?: () => Promise<void> }) {
  ...
  const activeDailyCursor = activeDailyLearningSession?.phase === "test" ? activeDailyLearningSession.testIndex : activeDailyLearningSession?.studyIndex ?? 0;
  const activeDailyRemaining = activeDailyLearningSession?.phraseIds.slice(activeDailyCursor).filter((id) => phraseIds.has(id)).length;
