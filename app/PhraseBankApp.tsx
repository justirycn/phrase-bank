...
function Brand() {
  return <div className="brand"><span className="brand-mark">P</span><div><b>Phrase Bank</b><small>我的英语语言块</small></div></div>;
}

function Empty({ title, detail, action }: { title: string; detail: string; action?: React.ReactNode }) {
  return <div className="empty"><div className="empty-glyph">“ ”</div><h3>{title}</h3><p>{detail}</p>{action}</div>;
}
...
export function PhraseBankApp({ repository }: { repository?: Repository }) {
  ...
  return <div className="app-shell">
    ...
    {screen !== "review" && <nav className="bottom-nav" aria-label="主导航">
      <button className={screen === "home" ? "active" : ""} onClick={() => go("home")}><span>⌂</span>复习</button>
      <button className={screen === "library" ? "active" : ""} onClick={() => go("library")}><span>▤</span>句库</button>
      <button className="add-nav" aria-label="添加" onClick={() => go("add")}><span>＋</span>添加</button>
      <button className={screen === "settings" ? "active" : ""} onClick={() => go("settings")}><span>⚙</span>设置</button>
    </nav>}
  </div>;
}
...
function Home({ phrases, dueCount, categories, onReview, onAdd }: { phrases: Phrase[]; dueCount: number; categories: Category[]; onReview: () => void; onAdd: () => void }) {
  ...
  return <>
    ...
    <section className="hero-card"><div className="eyebrow">TODAY'S PRACTICE</div><h1>{dueCount ? <>今天有 <em>{dueCount}</em> 条<br />语言块等你复习</> : <>今天的复习<br />已经完成了</>}</h1><p>{dueCount ? "先想意思，再让英文自然浮现。" : "积累一点，明天继续让表达更自然。"}</p><button className="primary" onClick={dueCount ? onReview : onAdd}>{dueCount ? "开始今日复习  →" : "收藏新的表达  ＋"}</button></section>
    <div className="section-title"><div><span>最近收藏</span><small>RECENT PHRASES</small></div><button onClick={onAdd}>＋ 添加</button></div>
    ...
    <aside className="tip"><span>✦</span><div><b>小提示</b><p>收藏完整表达，而不是孤立单词。比如记住 “I’ll get back to you.”</p></div></aside>
  </>;
}
...
function AddPhrase({ categories, onSave, onCancel }: { categories: Category[]; onSave: (input: PhraseInput) => Promise<void>; onCancel: () => void }) {
  ...
  return <><header className="screen-head"><button className="icon-button" onClick={onCancel} aria-label="返回">‹</button><div><h1>收藏语言块</h1><p>Save a phrase you’ll actually use.</p></div></header>
    <form className="phrase-form" onSubmit={submit}>
      ...
      <button className="more-button" type="button" onClick={() => setMore(!more)}>{more ? "收起选填内容" : "＋ 添加我的例句或来源"}</button>
...
function Library({ phrases, categories, onDelete, onAdd }: { phrases: Phrase[]; categories: Category[]; onDelete: (id: string) => Promise<void>; onAdd: () => void }) {
  ...
  return <><header className="top"><div><h1>我的句库</h1><p>让收藏慢慢变成可以调用的表达。</p></div><button className="round-add" onClick={onAdd}>＋</button></header>
    <div className="search"><span>⌕</span><input aria-label="搜索语言块" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索英文或中文…" /></div>
...
function Review({ phrases, onBack, onGrade }: { phrases: Phrase[]; onBack: () => void; onGrade: (id: string, result: ReviewResult) => Promise<void> }) {
  ...
  if (!phrase) return <div className="review-done"><div className="done-mark">✓</div><h1>今天完成了</h1><p>复习了 {index} 条语言块。主动回忆一次，就离自然表达更近一点。</p><div className="summary"><span>不会 <b>{counts.again}</b></span><span>模糊 <b>{counts.hard}</b></span><span>掌握 <b>{counts.good}</b></span></div><button className="primary" onClick={onBack}>回到首页</button></div>;
  return <div className="review-screen"><header><button className="icon-button" onClick={onBack} aria-label="退出复习">×</button><div className="progress"><span style={{ width: `${((index + (revealed ? .5 : 0)) / phrases.length) * 100}%` }} /></div><small>{index + 1} / {phrases.length}</small></header><div className="review-card"><small>中文提示</small><h1>{phrase.chinese}</h1>{revealed ? <div className="answer"><i /><small>自然表达</small><h2>{phrase.english}</h2>{phrase.personalExample && <p>{phrase.personalExample}</p>}</div> : <><p>先在心里或开口说出英文</p><button className="reveal" onClick={() => setRevealed(true)} aria-label="显示英文答案">显示英文答案</button></>}</div>{revealed && <div className="grade-row"><button className="again" onClick={() => grade("again")}><b>不会</b><small>10 分钟后</small></button><button className="hard" onClick={() => grade("hard")}><b>模糊</b><small>明天再见</small></button><button className="good" onClick={() => grade("good")}><b>掌握</b><small>延长间隔</small></button></div>}</div>;
}
