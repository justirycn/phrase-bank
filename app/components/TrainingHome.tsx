...
export function TrainingHome({ dailySummary, dailyProgress, streak, weeklySummary, focusPhrases, learnedToday, nextLearningCount, themeName, activeLearning, activeRemaining, activeReview, reviewRemaining, dueCount, heatmapDays, heatmapError, onRetryHeatmap, onContinue, onStartLearning, onStartStandard }: {
  ...
  return <div className="training-home">
    ...
    <div className="training-entry">
      <button className="continue-start" onClick={onContinue}><span><AppIcon name="play" size={24} /><b>继续今日任务</b><small>{activeLearning ? "继续未完成的新句学习" : activeReview ? `继续未完成的复习 · 剩余 ${reviewRemaining ?? 0} 句` : dueCount > 0 ? `${dueCount} 句待复习` : nextLearningCount > 0 ? `下一组 ${nextLearningCount} 句新内容` : "今天的任务已完成"}</small></span><AppIcon name="forward" size={22} /></button>
