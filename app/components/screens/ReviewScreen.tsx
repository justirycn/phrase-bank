...
export default function Review({ phrases, onBack, onGrade }: { phrases: Phrase[]; onBack: () => void; onGrade: (id: string, result: ReviewResult, operationId: string) => Promise<void> }) {
  ...
  const grade = async (result: ReviewResult) => { if (!phrase || submittingRef.current) return; submittingRef.current = true; try { await onGrade(phrase.id, result, operationIdRef.current); } catch { return; } finally { submittingRef.current = false; } operationIdRef.current = operationId(); setCounts((old) => ({ ...old, [result]: old[result] + 1 })); setIndex((i) => i + 1); setRevealed(false); };
