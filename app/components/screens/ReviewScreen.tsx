...
export default function Review({ phrases, onBack, onGrade }: { phrases: Phrase[]; onBack: () => void; onGrade: (id: string, result: ReviewResult, operationId: string) => Promise<void> }) {
  const [queue] = useState(() => phrases);
  ...
  const grade = async (result: ReviewResult) => {
    if (!phrase || submittingRef.current) return;
    const generation = generationRef.current;
    const phraseId = phrase.id;
    submittingRef.current = true;
    try { await onGrade(phraseId, result, operationIdRef.current); }
    catch { return; }
    finally { if (generation === generationRef.current) submittingRef.current = false; }
    if (generation !== generationRef.current || queue[indexRef.current]?.id !== phraseId) return;
    operationIdRef.current = operationId();
    setCounts((old) => ({ ...old, [result]: old[result] + 1 }));
    indexRef.current += 1;
