...
export function WeeklySummary({ streak, summary, focusPhrases = [] }: { streak: TrainingStreak; summary: WeeklyTrainingSummary; focusPhrases?: WeeklyFocusPhrase[] }) {
  return <section className="weekly-summary" aria-labelledby="weekly-title">
    ...
    <div className="weekly-grid"><p><strong>{Math.floor(summary.activeSeconds / 60)}</strong><span>有效分钟</span></p><p><strong>{streak.current}</strong><span>连续天数</span></p><p><strong>{summary.masteredCount}</strong><span>本周掌握</span></p><p><strong>{summary.promotedCount}</strong><span>从模糊到掌握</span></p></div>
