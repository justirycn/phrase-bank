...
export function SpeakingPractice({ controller, onPause, onHome, onAgain }: {
  ...
  return <section className={`speaking-practice phase-${controller.phase}`}>
    <header className="practice-head"><button type="button" className="practice-pause" aria-label="保存进度并返回" onClick={onPause}><AppIcon name="close" size={22} /></button><span className="task-mode task-mode-review">今日复习 · 中文回忆</span><span><AppIcon name="clock" size={18} /> 第 {controller.index + 1} / {controller.total} 个</span><div className="practice-track" role="progressbar" aria-label="今日复习进度" aria-valuemin={0} aria-valuemax={controller.total} aria-valuenow={controller.index + 1}><i style={{ width: `${((controller.index + (answered ? .6 : 0)) / Math.max(1, controller.total)) * 100}%` }} /></div></header>
