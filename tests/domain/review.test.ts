...
describe("review scheduling", () => {
  ...
  it("schedules again in ten minutes and resets progress", () => {
    const phrase = { ...createNewPhrase({ english: "A", chinese: "甲", categoryId: "daily" }, now), reviewStep: 3 };
    const result = scheduleReview(phrase, "again", now);
    expect(result.phrase.reviewStep).toBe(0);
    expect(result.phrase.nextReviewAt).toBe("2026-08-07T08:10:00.000Z");
  });

  it("schedules hard in one day without advancing", () => {
  ...
  it("advances good answers through 3, 7, 14, 30 and 60 days", () => {
