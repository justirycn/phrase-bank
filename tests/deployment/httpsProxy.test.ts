...
describe("HTTPS reverse proxy", () => {
  it("checks the public HTTPS endpoint during deployment", () => {
    ...
    expect(remoteScript).toMatch(
      /docker compose up -d[\s\S]*docker compose up -d --no-deps --force-recreate caddy[\s\S]*docker compose exec -T caddy caddy validate --config \/etc\/caddy\/Caddyfile --adapter caddyfile[\s\S]*docker compose exec -T caddy caddy reload --config \/etc\/caddy\/Caddyfile --adapter caddyfile[\s\S]*for attempt/,
    );
