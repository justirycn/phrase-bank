...
The generated current measurement records stable application source tree `cf7faaea730d34249c9ce5b418909370d308ccdc`. The `current.sha` value in `metrics.json` is a runner-generated informational field and changes when the command is rerun after an evidence commit; README and tests intentionally do not pin it. Tests compare only the recorded source tree with `git rev-parse HEAD:app`, so evidence-only commits do not create a self-referential identity loop. Before creating any temporary directory or updating metrics, the runner rejects tracked or untracked changes under `app/`.

| Production metric | Baseline | Current | Change |
| --- | ---: | ---: | ---: |
| Authenticated home chunk | 157,491 B | 63,513 B | -59.7% |
| Initial JS set from manifests | 524,925 B | 503,822 B | -4.0% |
