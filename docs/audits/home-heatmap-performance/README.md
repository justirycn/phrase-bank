...
This is a Codex in-app browser run at a **390脳844 CSS viewport**. It is **not a real iPhone**, **not Safari**, and **not a public-network measurement**. No claim about real-device paint time, mobile radio latency, or public deployment performance is made here.

The explicit viewport remained 390脳844 after five browser zoom shortcuts, so the attempted 200% check did not produce a verifiable 200% text-zoom state. `04-home-zoom-attempt-no-effect.jpg` is retained as evidence of that limitation, not as a passing 200% result.
...
The generated current measurement records stable application source tree `73d873542b700969add2697edb0aa43c8d70be68`. The `current.sha` value in `metrics.json` is a runner-generated informational field and changes when the command is rerun after an evidence commit; README and tests intentionally do not pin it. Tests compare only the recorded source tree with `git rev-parse HEAD:app`, so evidence-only commits do not create a self-referential identity loop. Before creating any temporary directory or updating metrics, the runner rejects tracked or untracked changes under `app/`.

| Production metric | Baseline | Current | Change |
| --- | ---: | ---: | ---: |
| Authenticated home chunk | 157,491 B | 61,721 B | -60.8% |
| Initial JS set from manifests | 524,925 B | 492,375 B | -6.2% |
...
## 390脳844 CSS viewport findings

- Home rendered all 84 accessible heatmap cells in a computed 12-column 脳 7-row grid.
- The accessibility tree exposed labels such as `8鏈?1鏃ワ紝鏈涔燻 and future-day labels such as `8鏈?2鏃ワ紝鏈潵鏃ユ湡`.
...
- Clicking 鍙ュ簱 produced the real `姝ｅ湪鎵撳紑鍙ュ簱鈥 Suspense fallback before the lazy chunk resolved. The loaded library also had no horizontal overflow.
...
| --- | --- | ---: |
| `01-home-390x844-full.jpg` | Loaded full home and heatmap | 375脳1608 |
| `02-library-fallback-390x844.jpg` | Real non-home lazy fallback | 390脳844 |
| `03-library-loaded-390x844.jpg` | Loaded library | 375脳812 |
| `04-home-zoom-attempt-no-effect.jpg` | Zoom attempt; viewport unchanged | 375脳812 |
