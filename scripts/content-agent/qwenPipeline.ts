...
function generationMessages(category: string, coreQuota: number, options: PipelineOptions): QwenMessage[] {
  return [
    { role: "system", content: "你是英语口语课程内容设计师。只返回 JSON，不要 Markdown。内容必须自然、实用、准确，适合中国成年学习者。" },
    { role: "user", content: `为 ${category} 类别生成完整批次：精确 ${coreQuota} 个核心语言块，每个核心 2–3 个相关发散案例。总包最终必须为 600 核心、2000 条。CEFR 仅 A2/B1/B2。使用版本 ${options.version} 和质检版本 ${options.qualityVersion}。返回 '{"phrases":[...]}'。` },
  ];
}

function reviewMessages(category: string, coreQuota: number, batch: BatchResponse): QwenMessage[] {
  return [
    { role: "system", content: "你是独立审校员，不继承生成上下文。检查口语自然度、中英文一致性、实用性、重复、冒犯或危险内容。只返回 JSON。" },
    { role: "user", content: `独立审校 ${category} 批次，必须包含 ${coreQuota} 个核心。修正后返回 '{"status":"pass|fail","issues":[],"phrases":[...]}'。输入：${JSON.stringify(batch)}` },
  ];
}

function assertBatch(category: string, coreQuota: number, batch: BatchResponse) {
  ...
  if (cores !== coreQuota) throw new Error(`${category} 核心数量错误：${cores}`);
}
...
export async function buildQwenCandidate(options: PipelineOptions): Promise<SystemContentPackage> {
  ...
  for (const [category, coreQuota] of CATEGORY_QUOTAS) {
    const generated = parseJson<BatchResponse>(await options.client.complete(generationMessages(category, coreQuota, options)));
    assertBatch(category, coreQuota, generated);
    const reviewed = parseJson<ReviewResponse>(await options.client.complete(reviewMessages(category, coreQuota, generated)));
    if (reviewed.status !== "pass") throw new Error(`${category} 审校未通过`);
    assertBatch(category, coreQuota, reviewed);
    phrases.push(...reviewed.phrases);
  }
